...
class BluetoothViewController: UIViewController {
    ...
    func sendData(_ data: Data, uuidString:String, writeType: CBCharacteristicWriteType){
        guard let charateristic = BluetoothReaderMain.charDictionary[uuidString] else{
            print("error")
        ...
        BluetoothReaderMain.connectPeripheral.writeValue(data, for: charateristic, type: writeType)
        DispatchQueue.main.async {
    ...
    @IBAction func start(_ sender: UIButton) {
        guard BluetoothReaderMain.connectPeripheral != nil else{
            return
        }
        let uuid = "6E400003-B5A3-F393-E0A9-E50E24DCCA9E"
        guard BluetoothReaderMain.charDictionary[uuid] != nil else
        {
            return
        }
        self.BluetoothReaderMain.connectPeripheral.setNotifyValue(true, for: BluetoothReaderMain.charDictionary[uuid]!)
        var data = Data()
